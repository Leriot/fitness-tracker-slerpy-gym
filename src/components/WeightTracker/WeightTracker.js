...
const HEIGHT_M = 1.71; // Richard's height in metres
...
// ── Quick linear regression (for stat cards only) ─────────────────────────────
...
const WeightTracker = () => {
  ...
  const [regression2026, setRegression2026]     = useState(false);
  ...
  // ── Derived data ─────────────────────────────────────────────────────────────
  // Time-range filtered data for display + graphs
  const timeFilteredData = useMemo(() => {
    if (timeRange === 'ALL' || fitnessData.length === 0) return fitnessData;
    const daysMap = { '30D': 30, '90D': 90, '180D': 180, '1Y': 365 };
    const days = daysMap[timeRange];
    if (!days) return fitnessData;
    const lastDate = fitnessData[fitnessData.length - 1].dateObj;
    const cutoff = new Date(lastDate.getTime() - days * 86400000);
    return fitnessData.filter(d => d.dateObj >= cutoff);
  }, [fitnessData, timeRange]);

  // Regression data: 2026-only toggle overrides time filter for the regression calc
  const regressionData = useMemo(() => {
    if (!regression2026) return null; // null = use timeFilteredData
    return fitnessData.filter(d => d.dateObj.getFullYear() >= 2026);
  }, [fitnessData, regression2026]);

  // ── Stat card values ─────────────────────────────────────────────────────────
  const stats = useMemo(() => {
    if (fitnessData.length === 0) return null;
    const first = fitnessData[0];
    const last  = fitnessData[fitnessData.length - 1];
    const totalChange = last.weight - first.weight;
    const currentBMI  = last.bmi;

    // Weekly rate from regression on the relevant dataset
    const regSource = regressionData && regressionData.length >= 2
      ? regressionData
      : timeFilteredData;
    const reg = quickRegression(regSource);
    ...
    if (reg && reg.slope !== 0) {
      ...
      const targetDays = (effectiveTarget - reg.intercept) / reg.slope;
      const remaining = targetDays - lastDays;
      if (remaining > 0) daysToTarget = Math.ceil(remaining);
    }

    return { first, last, totalChange, currentBMI, weeklyRate, daysToTarget };
  }, [fitnessData, timeFilteredData, regressionData, effectiveTarget]);
      ...
      <div className="weight-tracker">
        ...
        {stats && (
          <div className="stat-cards">
            <StatCard
              ...
              value={stats.last.weight.toFixed(1)}
              unit="kg"
              sub={stats.last.date}
              color="#64b5f6"
            />
            <StatCard
              ...
              unit="kg"
              sub={`from ${stats.first.weight.toFixed(1)} kg`}
            ...
            <StatCard
              ...
              unit=""
              sub={bmiCategory(stats.currentBMI)}
              color={bmiColor(stats.currentBMI)}
            />
            <StatCard
              label="Weekly Rate"
              value={stats.weeklyRate != null ? (stats.weeklyRate >= 0 ? '+' : '') + stats.weeklyRate.toFixed(2) : '—'}
              unit="kg/wk"
              sub={regression2026 ? '2026 trend' : 'current range'}
            ...
            <StatCard
              ...
              unit="kg"
              sub={stats.daysToTarget != null ? `~${stats.daysToTarget} days away` : (stats.last.weight <= effectiveTarget ? '✓ Reached!' : 'trend moving away')}
              color="#e040fb"
            />
          </div>
        )}
        ...
        {hasData && (
          <div className="controls">
            ...
            {/* 2026 Regression Toggle */}
            <div className="control-group">
              <button
                className={`control-button toggle-button ${regression2026 ? 'active' : ''}`}
                onClick={() => setRegression2026(v => !v)}
                title="Compute regression using only 2026 data, but display all historical data"
              >
                {regression2026 ? '✓ 2026 Regression' : '2026 Regression'}
              </button>
            </div>
          </div>
        )}
        ...
        {hasData && (
          <div className="data-preview">
            <div className="table-header">
              <h2>Recent Data ({timeFilteredData.length} entries)</h2>
            ...
            <table>
              <thead>
                <tr>
                  <th>Date</th><th>Time</th><th>Weight (kg)</th><th>BMI</th>
                </tr>
              </thead>
              <tbody>
        ...
        {hasData && !showGraphs && (
          <div className="generate-graphs-section">
            <button
              className="control-button action-button"
              onClick={() => { if (hasEnough) { setShowGraphs(true); setError(null); } else { setError('Need at least 2 data points in selected range.'); } }}
              disabled={!hasEnough}
            >
              📊 Generate Graphs
            </button>
